...
pub async fn interruptible_straight<'a, T, E: From<InterruptError>>(
    notified: Notified<'a>,
...
{
    interruptible_straight(notified, f).await
}

pub async fn interruptible_sendable<'a, T, E: From<InterruptError>>(
    notified: Notified<'a>,
...
mod tests {
    ...
    impl Test {
        ...
        pub async fn g(self) -> Result<u8, MyError> {
            ...
            interruptible(notified, Box::pin(async move {
                Ok(123)
            })).await
        }
    ...
    fn check_interruptible_sendable() {
        ...
        let _: &(dyn Future<Output = Result<i32, InterruptError>> + Send) = &interruptible_sendable(notified, Box::pin(async move {
            Ok(123)
        }));
    }
}
